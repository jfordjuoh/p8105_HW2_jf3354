...
s_np = read_csv("snp.csv") %>%
  ...
  mutate(year = if_else(year >= 50,
                        ...
                        year + 2000)) %>%
  mutate(Month = month.name[as.numeric(Month)]) %>%
  select(s_np, year, Month, everything()) %>%
  arrange(s_np, year, Month)


  mutate(year = as.numeric(pull(s_np, year))) %>%

#the code I had that worked#
s_np = read_csv("snp.csv") %>%
  separate(date, c("Day", "Month", "year")) 
  mutate(year = as.numeric(pull(s_np, year))) %>%
  mutate(year = if_else(year >= 50,
                        year + 1900,
                        year + 2000)) %>%
  mutate(year = as.character(pull(s_np, year))) %>%
  mutate(Month = month.name[as.numeric(Month)]) %>%
  select(year, Month, everything()) %>%
  arrange(s_np, year, Month)
#end of code that I had that worked#
