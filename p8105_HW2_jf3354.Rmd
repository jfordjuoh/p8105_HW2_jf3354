...
##Problem 3
#ASKKKK: what they want: this should have rows for ethnicities and columns for year. How would I get the ethnicity as a row? Inintally I thought pivot widr but the dataset is already wide?
```{r}
...
select(-gender) %>%
select(year_of_birth, rank, count, everything()) %>%
arrange(year_of_birth, rank) %>%
  knitr::kable("pipe") 


#first figure out how to get oliva by itself (use filter?),
#we have to arrange it by rank and year (year should be a column)
#ethnicity should be a row
#we might have to pivot wider so that the ethnicity is a row

#knir\cable is what we have to use  
```

```{r}
#table for the males
#filter(pbn, gender == "MALE", rank == "1", year_of_birth == "2010")
#After filtering the dataset by gender (Male), rank (1), and by year (ran indvidually from year 2011-2016), the name Jayden was ranked first at least by one ethnicity from the years 2011-2015 which makes it the most popular name in this dataset.

filter(pbn, childs_first_name == "JAYDEN") %>%  
select(-gender) %>%
select(year_of_birth, rank, count, everything()) %>%
arrange(year_of_birth, rank) %>%
  knitr::kable("pipe") 
...
#Is the number of children with a name same as the count?
```{r}
babynamef = filter(babynamef, gender == "MALE", ethnicity == "WHITE NON HISPANIC", year_of_birth == "2016") %>%
p <- ggplot(babynamef , aes(x = rank, y = count)) + geom_point(alpha = 0.5)  %>%
